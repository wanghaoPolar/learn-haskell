...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok p n a = between (charTok p) (charTok n) a
...
eof = P (\input ->
  case input of
    Nil -> Result Nil ()
    otherwise -> ErrorResult $ ExpectedEof input
  )
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll l = satisfy (\char -> and $ (\f -> f char) <$> l)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny l = satisfy (\char -> or $ (\f -> f char) <$> l)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma p n a = betweenCharTok p n (sepby a $ charTok ',')